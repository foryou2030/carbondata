...
class GlobalDictionaryUtilTestCase extends QueryTest with BeforeAndAfterAll  {
  ...
  test("[issue-80]Global Dictionary Generation"){
    ...
    var rtn = GlobalDictionaryUtil.generateGlobalDictionary(CarbonHiveContext, carbonLoadModel, sampleRelation.cubeMeta.dataPath, false, false, null)
    assert( rtn === 1)
    ...
    rtn = GlobalDictionaryUtil.generateGlobalDictionary(CarbonHiveContext, carbonLoadModel, dimSampleRelation.cubeMeta.dataPath, false, false, null)
    assert( rtn === 1)
  }
}
